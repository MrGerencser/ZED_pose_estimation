...
def subtract_point_clouds_gpu(workspace_cloud, objects_cloud, distance_threshold=0.05):
    """Subtract object point clouds from workspace point cloud using GPU."""
    # If either cloud is empty, handle appropriately
    if workspace_cloud.size == 0:
        return workspace_cloud
    
    if objects_cloud.size == 0:
        # If no objects detected, return the workspace cloud as is
        return workspace_cloud
    
    # Convert to torch tensors if they aren't already
    device = torch.device('cuda' if torch.cuda.is_available() else 'cpu')
    
    if isinstance(workspace_cloud, np.ndarray):
        workspace_tensor = torch.tensor(workspace_cloud, dtype=torch.float32, device=device)
    else:
        workspace_tensor = workspace_cloud
        
    if isinstance(objects_cloud, np.ndarray):
        objects_tensor = torch.tensor(objects_cloud, dtype=torch.float32, device=device)
    else:
        objects_tensor = objects_cloud
    
    # Process in batches if there are many points
    max_points = 100000
    result_list = []
    
    for i in range(0, workspace_tensor.shape[0], max_points):
        batch = workspace_tensor[i:i+max_points]
        
        keep_mask = torch.ones(batch.shape[0], dtype=torch.bool, device=device)
        
        # For each object point, find workspace points within threshold
        for j in range(0, objects_tensor.shape[0], max_points):
            obj_batch = objects_tensor[j:j+max_points]
            
            # Ensure the object batch is not empty
            if obj_batch.shape[0] == 0:
                continue
                
            # Calculate pairwise distances efficiently
            # Reshape for broadcasting: [batch_size, 1, 3] - [1, obj_batch_size, 3]
            dists = torch.sum(
                (batch.unsqueeze(1) - obj_batch.unsqueeze(0)) ** 2, 
                dim=2
            )
            
            # Now dists has shape [batch_size, obj_batch_size]
            # Check that dimension 1 is not empty before calling min
            if dists.shape[1] > 0:  # This check ensures obj_batch_size > 0
                min_dists, _ = dists.min(dim=1)
                keep_mask = keep_mask & (min_dists > distance_threshold**2)
        
        # Keep only points that are far enough from any object point
        filtered_batch = batch[keep_mask]
        result_list.append(filtered_batch.cpu().numpy())
